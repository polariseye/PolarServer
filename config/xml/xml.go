...
import (
	...
	"github.com/polariseye/polarserver/config/xml/x2j"
)
...
func (this *ConfigContainer) OutAll() {

	outInterface("", this.data)
}

func (this *ConfigContainer) outInterface(separator string, data interface{}) {

	mpData, isOk := data.(map[string]interface{})
	if isOk {
		for mpKey, mpVal := range mpData {
			fmt.Println(separator, "map :", mpKey)
			this.outInterface(separator+" ", mpVal)
		}
	...
	if isArr {
		fmt.Println(separator, "array——》value:")
		for _, item := range arr {
			this.outInterface(separator+" ", item)
		}

		return
	}

	fmt.Println(separator, data)
}
